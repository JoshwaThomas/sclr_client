    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                ...
                const freshSFM = freshResponse.data.filter(user => user.procategory === 'SFM' && user.classAttendancePer === 0 && user.action === 0 && user.acyear === curacyear.acyear);
                const renewalSFM = renewalResponse.data.filter(user => user.procategory === 'SFM' && user.classAttendancePer === 0 && user.action === 0 && user.acyear === curacyear.acyear);
                const totalfilter = freshSFM.length + renewalSFM.length;
                const work = totalsfm - totalfilter;
                setTotalwork(work);
                setTotaldata(totalsfm);
                const combinedUsers = [...freshSFM, ...renewalSFM];
