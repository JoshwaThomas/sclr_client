...
function AttendMoralSFW() {
        ...
        const calculatePercentage = () => {
            const updatedAttendancePer = users.reduce((acc, user) => {
                ...
                if (user.semester === "I" || user.semester === "II") {
                    acc[user.registerNo] = totalCurrAttendance > 0
                        ? ((currAttendance / totalCurrAttendance) * 100).toFixed(2)
                        : '0';
                } else {
                    const total = totalPrevAttendance + totalCurrAttendance;
                    acc[user.registerNo] = total > 0
                        ? (((prevAttendance + currAttendance) / total) * 100).toFixed(2)
                        : '0';
                }
                return acc;
            }, {});
    ...
    return (
        <div className="p-6">
            ...
            {/* Table */}
            <div className="overflow-x-auto rounded-lg shadow ring-1 ring-black ring-opacity-5 mt-6">
