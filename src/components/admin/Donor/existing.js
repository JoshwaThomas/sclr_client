...
function Existing() {
    ...
    useEffect(() => { setBalance(amount); setZakkathbal(zakkathamt) }, [amount, zakkathamt]);

    useEffect(() => { setBalance(amount) }, [amount]);
        ...
        try {
            ...
            if (response.data.success) {
                ...
                const postData = {
                    ...
                    scholtype, scholdate, pan, acyear, receipt, did,
    ...
    return (
        <div className='p-6'>
            <h1 className="text-xl mb-6 font-semibold bg-gray-600 p-3 rounded text-white"> Existing Donor </h1>
            <form onSubmit={Submit} className='space-y-8 font-semibold'>
                <div className="border border-black p-6 rounded-xl space-y-10">
                    <div onChange={(e) => fetchPanList(e)} className='w-full'>
                        <div ref={dropdownRef} className="relative">
                            <div>
                                <label className="block mb-3 font-semibold text-slate-700">Name or ID :</label>
                                <input
                                    type="text" required
                                    value={searchTerm}
                                    onChange={(e) => { setSearchTerm(e.target.value); setIsDropdownOpen(true) }}
                                    onClick={() => setIsDropdownOpen(true)}
                                    className="p-2 border border-black rounded-md text-slate-950 w-full"
                                    placeholder="Search Donor"

                                />
                                {isDropdownOpen && (
                                    <ul className="absolute z-10 w-full p-2 border rounded-md bg-white max-h-60 overflow-y-auto">
                                        {filteredPanList.length > 0 ? (
                                            filteredPanList.map((panItem) => (
                                                <React.Fragment key={panItem.did}>
                                                    <li
                    ...
                    <div className='flex justify-end'>
                        <button type='submit' className=' p-2 border w-24 text-center text-white font-bold rounded-md  hover:bg-orange-700 bg-orange-500'>Submit</button>
                    </div>
                </div>
            </form>
        </div>
